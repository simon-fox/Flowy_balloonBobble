...
MyGame = ig.Box2DGame.extend({
	...
	spawnInNewItem: function(){
		console.log( ig.game.getEntitiesByType( EntityBalloon_ropeSegment ).length );
		//here we push a new balloon to be inflated. 
		//soon we will add other items than balloons
		var randNum = Math.random();
		if( randNum <= 0.15 ){
			//spawn in a projectile
			ig.game.spawnEntity( EntityCatProjectile , 86 , 640 );
		}
		else if( randNum >= 0.85 ){
			ig.game.spawnEntity( EntityStoneObstacle , 86 , 640 );
		}
